...
Veremos ahora la complejidad de dos funciones distintas, $\proc{Zombieland}$ y $\proc{Recorridos}$, ambas funciones imprescindibles para nuestra solución.\\
Empezemos por $\proc{Zombieland}$, la cual toma los parámetros previamente procesados y organizados de la ciudad (inicializada recorriendo la entrada $\mathcal{O}(n*m}$ seteando los atributos de las distintas posiciones de la matriz, indicando también las calles que resultan inválidas $\mathcal{O}(n*m)$),el punto de partida, la posición del búnker y la cantidad de soldados, usando los datos levantados de la entrada en $\mathcal{O}(1)$. Con estos datos se procede a inicializar una lista que indicará las esquinas marcadas y a setear en tope la cantidad de soldados con la que pretendemos llegar al búnker. Partiendo de ese valor se ciclará tantas veces como seas necesarias hasta encontrar un camino o determinar que no hay camino posible. Notemos que en el peor caso tendríamos que ciclar $s-1$ solo para encontrarnos con que no hay un camino posible. En cada iteración, se ejecuta la función $\proc{Recorridos}$ partiendo de la primera esquina de la lista de marcadas. Por ahora asumamos que dicha función tiene un costo de $\mathcal{O}(n*m)$. Siendo así vemos que en el peor de los casos, nuestra función itera $s-1$ veces y cada una con un costo de $\mathcal{O}(n*m)$ llegando asi a un costo total de $\mathcal{O}(s*n*m)$.\\
Pasemos ahora a la función $\proc{Recorridos}$. Esta parte de una posición tomada como parámetro y analiza las distintas posibilidades de movimientos $\mathcal{O}(1)$, revisando siempre que estos sean viables (si la cantidad de zombies no pasa la cota de soldados a perder $\mathcal{O}(1)$). Si en efecto es un movimiento válido, se actualiza el tope y los atributos de la esquina $\mathcal{O}(1)$ y se llama a la recursión sobre la esquina destino, armando así los recorridos.\\
Un aspecto crucial de esta función es no permite repetir caminos y como consecuencia de esto una esquina puede ser visitada un número acotado de veces. Como en el peor caso se recorre toda la ciudad un número fijo de veces, diremos que la complejidad es $\mathcal{O}(n*m)$.\\
Hasta ahora la complejidad de nuestro algoritmo parecería ser $\mathcal{O}(n*m)$. Para finalizar se procede a armar el resultado y a mostrarlo. Para lo primero partimos desde el búnker y haciendo uso del atributo origen, que nos indica desde que esquina se llego a la esquina en cuestión, ``deshacemos'' el camino realizado. Por cada retroceso se inserta la esquina en una nueva lista y asi sucesivamente hasta llegar al punto de partida, en un proceso que en el peor caso (recorri toda la ciudad) toma $\mathcal{O}(n*m)$. Para mostrar el resultado se recorre la lista armada anteriormente y se muestra cada esquina recorrida junto con la cantidad de soldados con la cuál finalizamos el recorrido $\mathcal{O}(n*m)$.\\
Luego comprobamos que la complejidad es $\mathcal{O}(s*n*m)$
