...
\Procname{$\proc{Zombieland}(entrada$ $estándar)$}{
 ...
 \\
  ...
  //Entonces el procedimiento es recorrer las n líneas levantando los datos, esto lo consideramos $\mathcal{O}(1)$\\ para las n ciudades, debido a que la creacion del tipo ciudad y la asignación 
  de sus valores los \\ tomamos como $\mathcal{O}(1)$
 \\ \\
 $calcular$_$costo$_$de$_$salvacion(cities,n)$ // $\mathcal{O}(n)$
 \\ \\
$ordenar$_$por$_$costo(cities)$ //de menor a mayor $\mathcal{O}(n.log n)$
 \\ \\
 $ciudades$_$salvadas = zombie$_$goloso(cities,n,p)$ // $\mathcal{O}(n)$
 \\ \\
 $ordenar$_$por$_$nombre}(cities)$ // $\mathcal{O}(n.log$ $n)$
 \\ \\
 ...
 \\ \\
 //Esto se hace en $\mathcal{O}(n)$ dado que es recorrer las n ciudades y, en base a su campo "salvar"\\ determinar si se debe \\
...
\\
\begin{codebox}
\Procname{$int$ $\proc{zombie$_$goloso}(int$ $n, int$ $p)$}
\\
i $\longleftarrow$ 0\\
sum $\longleftarrow$ 0\\
\\
